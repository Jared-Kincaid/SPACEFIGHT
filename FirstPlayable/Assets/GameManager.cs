...
public class GameManager : MonoBehaviour 
{
	...
	private const string typeName = "DigiPenSpacefight";
	private string gameName = "Spacefight Lobby\n";
	...
	void SpawnPlayers()
	{
		...
		if(networkGame)
		{
			playerList = GameObject.FindGameObjectsWithTag("Ship");
		}
		...
		for(int i = 0; i < numPlayers; ++i)
		{
			...
			SpriteRenderer newShip = null;
			if(!networkGame)
			{
				newShip = Instantiate(shipPrefab, playerPosition, playerRotation) as SpriteRenderer;
			}
			else
			{
				if(playerList != null)
				{
					newShip = playerList[i].GetComponent<SpriteRenderer>();
					newShip.transform.position = playerPosition;
					newShip.transform.rotation = playerRotation;
				}
			}
	...
	void GameSelectState()
	{
		...
		if(GUI.Button(new Rect(bPosX, bPosY, bWidth, bHeight), "Host Game"))
		{
			...
			StartServer();
			Vector3 playerPosition = new Vector3(1000, 1000, 1000);
			Network.Instantiate(shipPrefab, playerPosition, shipPrefab.transform.rotation, 0);
		}
	...
	[RPC] void StartNetworkGame()
	{
		networkGameStarted = true;

		if (networkView.isMine)
		{
			networkView.RPC("StartNetworkGame", RPCMode.OthersBuffered);
		}
	}
	...
	void HostGameState()
	{
		...
		if(GUI.Button(new Rect(bPosX, bPosY, bWidth, bHeight), "Back"))
		{
			SoundManager.Call_SoundEffect (selectSound, 0.5f);
			currentGameState = "GameSelect";
			GameObject[] killShips = GameObject.FindGameObjectsWithTag("Ship");
			foreach(GameObject curShip in killShips)
			{
				Destroy(curShip);
			}
		}
	...
	{		
		...
		if(Network.isClient)
		{
			GUI.Button(new Rect(bSpace, bSpace, screenRight - 2 * bSpace, 4 * bHeight + 3 * bSpace), "Waiting for Server...");
		...
		{
			for (int i = 0; i < hostList.Length && i < 4; i++)
			{
				if (GUI.Button(new Rect(bPosX, bSpace + ((bHeight + bSpace) * i), bWidth, bHeight), hostList[i].gameName))
				{
		...
		if(GUI.Button(new Rect(bPosX, screenBottom - 2 * (bHeight + bSpace) - numButtons + 1, bWidth, bHeight), "Refresh"))
		{
			RefreshHostList();
		}

		if(GUI.Button(new Rect(bPosX, screenBottom - bHeight - bSpace - numButtons + 1, bWidth, bHeight), "Back"))
		{
			SoundManager.Call_SoundEffect (selectSound, 0.5f);
			currentGameState = "GameSelect";
			GameObject[] killShips = GameObject.FindGameObjectsWithTag("Ship");
